...
server <- function(input, output) {
	...
	observeEvent(c(my$long, my$lat), {
		...
		# minx <- floor(min(data$pasture_eaten_raw, data$pasture_eaten_min, data$pasture_eaten_max))
		# maxx <- ceiling(max(data$pasture_eaten_raw, data$pasture_eaten_min, data$pasture_eaten_max))
	...
	output$stacked_histogram <- renderPlotly({
		...
		gridcolor <- toRGB("lightgrey")
		targetcolor <- toRGB("blue")
		textsize <- 15
		...
		p[[3]] <- samppdf %>% 
		  ...
		  add_segments(x=~qrlower[1], xend=~qrlower[1], y=yrange[1], yend=yrange[2], color=I(targetcolor), opacity=0.2, showlegend=FALSE) %>% 
		  add_segments(x=~qrupper[1], xend=~qrupper[1], y=yrange[1], yend=yrange[2], color=I(targetcolor), opacity=0.2, showlegend=FALSE) %>% 
		  add_segments(x=~qr[1], xend=~qr[1], y=yrange[1], yend=yrange[2], color=I(targetcolor), showlegend=FALSE) %>% 
		  add_text(x=xrange[1]+(xrange[2]-xrange[1])*0.95, y=yrange[2]*0.9, text=~radius, textposition="middle left", textfont=list(size=textsize)) %>% 
		  add_text(x=~qr[1]+0.15, y=yrange[2]*0.8, text=~sprintf("%.1f t", q), color=I(targetcolor), textposition="middle right", textfont=list(size=textsize)) %>% 
		  layout(
		    showlegend=FALSE,
		    yaxis=list(range=yrange, showticklabels=FALSE),
		    xaxis=list(title="<b>Pasture and Crop Eaten, tonnes DM per ha</b>", gridcolor=I(gridcolor), range=xrange, dtick=1)
		  )
		p[[2]] <- samppdf %>% 
		  ...
		  layout(
		    showlegend=FALSE,
		    yaxis=list(title="<b>Number of Farms</b>", range=yrange, showticklabels=FALSE),
		    xaxis=list(gridcolor = I(gridcolor), range=xrange, dtick=1)
		  )
		p[[1]] <- samppdf %>% 
