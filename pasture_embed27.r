...
server <- function(input, output) {
	...
	output$season_selector <- renderUI({
	  ...
	  selectInput("season", p("Production season?", style="line-height: 0.0; margin-top: 30px;"), my$season_here, my$season_default )
	})
	...
	output$stacked_histogram <- renderPlotly({
		...
		p[[3]] <- p[[3]] %>%   
		  layout(
		    ...
		    xaxis=list(
		      title="<b>Pasture and Crop Eaten (tDM/ha)</b>", 
	...
	output$plot_title <- renderText({
	  req(my$name)
	  paste("<b>Pasture and Crop Eaten Near", my$name, "</b>")
	})
