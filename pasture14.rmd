...
css <- ".selectize-input { font-size: 14px; line-height: 14px;} .selectize-dropdown { font-size: 14px; line-height: 14px; }"
...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      width=7,
      leafletOutput("map", width="100%", height=540), # can manipulate size here
...
output$stacked_histogram <- renderPlot({
  ...
  isolate({
    ...
    stacked_histogram <- ggplot() +
      ...
      # theme_cowplot() +
      theme_stata(base_size=14, scheme="s2color") +
