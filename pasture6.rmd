...
```{r input_pane, echo=FALSE, eval=TRUE}
  ...
  observeEvent(c(my$season_sel, input$season), {

    cat(file=stderr(), "\n")
    cat(file=stderr(), paste("change of season =", input$season), "\n")
  ...
  observeEvent(c(my$soil_sel, input$soil), {
...
calc <- eventReactive(c(my$elev_sel, input$elev), {

  cat(file=stderr(), "\n")
  cat(file=stderr(), paste("change of elev"), "\n")

  cat(file=stderr(), paste("analyse for location =", my$long, my$lat), "\n")
  cat(file=stderr(), paste("analyse for season =", input$season), "\n")
  cat(file=stderr(), paste("analyse for soil ="), paste(input$soil), "\n")
  cat(file=stderr(), paste("analyse for elev =", input$elev), "\n")

  my$name <- paste("You (", season_all[input$season], ")", sep="")

  data_sel <-  my$data_sel %>% 
    filter(season == input$season) %>% 
    filter(soil %in% input$soil) %>%
    filter(elev_fact == input$elev)
