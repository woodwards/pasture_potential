...
/* font doesnt work */
/* https://stackoverflow.com/questions/29274501/r-markdown-changing-font-size-and-font-type-in-html-output/29298407 */
body{
  font-family: Arial;
  font-size: 16pt;
}
...
server <- function(input, output) {
	...
	observeEvent(c(my$long, my$lat), {
		...
		n <- unlist(purrr::map(my$season_here, function(u) sum(u==data$season)))
	...
	output$stacked_histogram <- renderPlot({
		...
		samppdf <- calc()$samppdf # get data for histogram when calc() changes # makes a slow copy！
	  # temp <- length(calc()) # react to calc()
