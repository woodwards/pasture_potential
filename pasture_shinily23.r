	...
	output$stacked_histogram <- renderPlotly({
		...
		p[[3]] <- samppdf %>% 
		  ...
		  layout(
		    ...
		    xaxis=list(title="<b>Pasture and Crop Eaten, tonnes DM per ha</b>", gridcolor = gridcolor, range=xrange, dtick=1)
		  )
		p[[2]] <- samppdf %>% 
		  ...
		  add_histogram(x=~pasture, color=~radius, opacity=0.5, xbins=xbins) %>% 
		  layout(
		    yaxis=list(title="<b>Number of Farms</b>", range=yrange),
		    xaxis=list(gridcolor = gridcolor, range=xrange, dtick=1)
		  )
		p[[1]] <- samppdf %>% 
