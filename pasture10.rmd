...
output$nitrogen_checkbox <- renderUI({
  cat(file=stderr(), paste("render nitrogen checkbox"), "\n")
  checkboxInput("adjust", strong("Adjust for Nitrogen?"), value=my$adjust)
})
...
#### react to change of location or nitrogen adjustment ####
...
observeEvent(c(my$long, my$lat, my$adjust), {
  ...
  if (my$adjust){
    ...
    data <- data %>%
      ...
      mutate(
        ...
        pasture_eaten = pasture_eaten_raw - nitrogen_applied * nitrogen_slope
        )
  }
  ...
  my$breaks <- seq(floor(min(my$data$pasture_eaten)), ceiling(max(my$data$pasture_eaten)), 1)
...
output$stacked_histogram <- renderPlot({
  ...
  isolate({
    ...
    if (my$adjust){
      title_string <- paste("Pasture Eaten near", my$name, "(Nitrogen Corrected)")
    } else {
